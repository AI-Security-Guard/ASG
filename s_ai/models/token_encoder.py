...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.return_intermediate:
            # hs: [L, B, Q, C], att: [L, B, H, Q, K]
            return torch.stack(intermediate), torch.stack(intermediate_att)

        # keep an extra leading dim to match DETR style: [1, ...]
        # hs: [1, B, Q, C], att: [1, B, H, Q, K]
...
class TransformerDecoderLayer(nn.Module):
    ...
    @staticmethod
    def _normalize_att_shape(att: Tensor, num_heads: int) -> Tensor:
        """
        Normalize attention weight shape to [B, H, Q, K].
        PyTorch may return:
          - [B*H, Q, K]
          - [H, B, Q, K]
          - [B, H, Q, K]
        ...
        if att.dim() == 3:
            ...
            h = num_heads
            b = bh // h
            att = att.view(b, h, q, k)
        elif att.dim() == 4:
            # Could be [H, B, Q, K] → [B, H, Q, K]
            if att.shape[0] == num_heads:
                att = att.permute(1, 0, 2, 3).contiguous()
            # else already [B, H, Q, K]
        return att
    ...
    def forward_post(self, tgt, memory,
        ...
        # ⬇️ per-head attention 반환 (average_attn_weights=False)
        ...
        att = self._normalize_att_shape(att, self.multihead_attn.num_heads)  # → [B, H, Q, K]
